...
//                'presell_delivery_time' => array('elt', 1522293275)
   ...
   public function bargainOperation(){
       ...
       try{
           ...
           $address = new Address();
           $list = $bargain_launch->getQuery($condition_close_launch, 'launch_id,end_time, bargain_money,sku_id, launch_id, receiver_mobile, receiver_province, receiver_city, receiver_district, receiver_address, receiver_zip, receiver_name, uid', '');
           foreach($list as $item){
               ...
               $order_id = $order_service->orderCreateBragain($out_trade_no, $item['sku_id'].":1",$item['receiver_mobile'], $item['receiver_province'], $item['receiver_city'],$item['receiver_district'],$address_info, $item['receiver_zip'], $item['receiver_name'], $item['bargain_money'], $item['uid']);
               $bargain_launch = new NsPromotionBargainLaunchModel();

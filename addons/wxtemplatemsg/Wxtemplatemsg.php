...
class Wxtemplatemsg extends \addons\Addons
{
    ...
    public function orderDistributionSuccess($params = []){
        ...
        if(!empty($nfx_promoter_info)){
            $nfx_commission_money = $nfx_commission_distribution -> getInfo(["order_id"=>$params['order_id'], "promoter_id"=>$nfx_promoter_info['promoter_id']], "commission_money");
            $keyword3 = !empty($nfx_commission_money) ? $nfx_commission_money : 0.00;
        }
